    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->avgDiamondButton, SIGNAL(clicked(bool)), this, SLOT(avgDiamondClicked()));

    connect(ui->budgetButton, SIGNAL(clicked(bool)), this, SLOT(budgetClicked()));
    ...
    connect(ui->plusButton, SIGNAL(clicked(bool)), this, SLOT(plusClicked()));
...
void MainWindow::minusClicked()
{
    ...
    for (const QModelIndex &index : indexes) {
        ...
        int diamondIndex = table->modelIndexByDiamondIndex(clickedIndex);
        table->removeRow(diamondIndex);
        ui->tableView->update();
    }
}
...
{
    Diamond newDiamond;
    newDiamond.index = table->rowCount() + 1;

    DiamondDialog *dd = new DiamondDialog(&newDiamond);
    int result = dd->exec();
    if (result == QDialog::Accepted) {
        table->addDiamond(newDiamond);
